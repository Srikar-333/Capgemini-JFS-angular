...
//For authentication
	...
	private ResponseEntity<?> authenticateCustomer(@RequestBody AuthenticationRequest authenticationRequest){
		...
		try {
			...
			return ResponseEntity.ok(new AuthenticationResponse("please check the correct details of user: "+ username));
		}
...
//To find all addons
		...
		public AddOns[] getAddOn(){
			ResponseEntity<AddOns[]> response = restTemplate.getForEntity("http://localhost:8090/findAllAddOns", AddOns[].class);
			AddOns[] addOns = response.getBody();
			return addOns.clone();
		}
